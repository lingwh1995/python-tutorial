...
1. 列表推导式（List Comprehensions）
     列表推导式 是 Python 中一种非常强大且优雅的生成列表的方式。它允许使用简短的语法根据现有的可迭代对象创建一个新的列表，通常可以用
     一行代码替代多行的 for 循环。列表推导式不仅代码简洁，而且在 Python 内部是通过 C 语言优化的，执行速度通常比等价的 for 循环更快
2. 生成器表达式（Generator Expressions）
     生成器表达式 是列表推导式的惰性求值版本。它在语法上与列表推导式非常相似，唯一的区别在于它使用圆括号 () 而非方括号 []。生成器表达
     式不会立即生成一个包含所有结果的列表，而是返回一个生成器（generator）对象，该对象会根据需要逐个地产生结果。这种惰性计算的特性使
     得生成器表达式在处理大规模数据或无限序列时非常有用，因为它可以显著节省内存。常和 sum()、max()、min() 等配合使用。
3. 内置函数
   Python提供了许多内置函数（如sum()、max()、all()、any()等），它们由C语言实现，通常比手写的for循环更高效。在可能的情况下，应优先
   使用这些内置函数或标准库工具（如itertools模块）来替代显式循环，以减少解释器开销。
